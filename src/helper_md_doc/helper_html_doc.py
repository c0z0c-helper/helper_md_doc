...
def clean_html_for_pandoc(html_text: str) -> str:
    ...
    html_text = re.sub(r'<script[^>]*katex[^>]*>.*?</script>', '', html_text, flags=re.IGNORECASE | re.DOTALL)
    html_text = re.sub(r'<script[^>]*mermaid[^>]*>.*?</script>', '', html_text, flags=re.IGNORECASE | re.DOTALL)
